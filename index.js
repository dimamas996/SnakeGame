export class SnakeGame {
  constructor(isMobile = true) {
    ...
    this.action = null
    this.gameSpeed = 7
    this.message = this.isMobile ? 'Swipe in any direction to start the game.' : 'Press any arrow button to start the game.'
  ...
  #init() {
    window.addEventListener('resize', () => {
      ...
      length = length - length * 0.06 - ((length - length * 0.06) % this.quantityOfSqInRow)
    ...
    if (this.isMobile) {
      ...
      this.BTN.addEventListener('touchstart', () => {
        this.BTN.classList.toggle('button--touchstart')
      })
  ...
  gameAction() {
    ...
    if (this.snakeHeadPos.x > this.quantityOfSqInRow - 1) this.snakeHeadPos.x = 0
    if (this.snakeHeadPos.x < 0) this.snakeHeadPos.x = this.quantityOfSqInRow - 1
    if (this.snakeHeadPos.y > this.quantityOfSqInRow - 1) this.snakeHeadPos.y = 0
    if (this.snakeHeadPos.y < 0) this.snakeHeadPos.y = this.quantityOfSqInRow - 1
  }
  ...
  speedChange() {
    ...
    if (newSpeed !== this.gameSpeed) {
      ...
      this.action = setInterval(this.gameAction.bind(this), 1000 / this.gameSpeed)
    }
  }
  ...
  animation() {
    ...
    this.CTX.fillText(`${this.title}`, this.CNVS.width / 2, this.CNVS.height * 0.35)

    this.CTX.textAlign = 'center'
    this.CTX.fillText(`${this.message}`, this.CNVS.width / 2, this.CNVS.height * 0.65)
